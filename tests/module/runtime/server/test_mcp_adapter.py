...
class TestClockMCPAdapter:
    async def test_start_lists_get_time_tool(self):
        adapter = MCPAdapter("svc.clock@1", _cmd("mars.runtime.agents.clock_agent"))
        try:
...
class TestSympyMCPAdapter:
    async def test_start_lists_solve_math_tool(self):
        adapter = MCPAdapter("svc.sympy@1", _cmd("mars.runtime.agents.sympy_agent"))
        try:
            tools = await adapter.start()
            names = [t.name for t in tools]
...
class TestSciPyMCPAdapter:
    async def test_start_lists_solve_scipy_tool(self):
        ...
        adapter = MCPAdapter("svc.scipy@1", _cmd("mars.runtime.agents.scipy_agent"))
        try:
            tools = await adapter.start()
            names = [t.name for t in tools]
