"""Unit tests for mars.server.service_manager."""
...
from mars.server.services.registry import AgentSpec
...
def _make_spec(name: str = "status", command: str = "mars-agent-status --server {server}",
...
class TestLaunchProvider:
    ...
    def test_returns_pid_on_success(self, mock_popen: MagicMock) -> None:
        ...
        spec = _make_spec()
        pid, workdir = _launch_provider(spec, "localhost:7432")

        assert pid == 12345
        assert workdir == Path("artifacts") / "status"
    ...
    def test_creates_workdir(self, mock_popen: MagicMock, tmp_path: Path) -> None:
        ...
        _launch_provider(spec, "localhost:7432")
    ...
    def test_server_address_substituted_in_command(self, mock_popen: MagicMock) -> None:
        ...
        spec = _make_spec(command="mars-agent-status --server {server}")
        _launch_provider(spec, "192.168.1.100:7432")
    ...
    def test_extra_args_appended(self, mock_popen: MagicMock) -> None:
        ...
        spec = _make_spec()
        _launch_provider(spec, "localhost:7432", extra_args=["--verbose"])
...
class TestAutoSpawnFreeAgents:
    @patch("mars.server.service_manager._launch_provider")
    @patch("mars.server.service_manager.all_specs")
    def test_spawns_only_free_agents(
        ...
        pids = _auto_spawn_free_providers("localhost:7432")
        assert pids == [1234]
