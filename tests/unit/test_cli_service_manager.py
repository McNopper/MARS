"""Unit tests for mars.cli.service_manager."""
...
from mars.services.registry import AgentSpec
...
class TestLaunchServiceAgent:
    ...
    def test_returns_pid_on_success(self, mock_popen: MagicMock) -> None:
        ...
        spec = _make_spec()
        pid, workdir, via_module = _launch_service_agent(spec, "localhost:7432")

        assert pid == 12345
        assert via_module is False
    ...
    @patch("subprocess.Popen")
    def test_falls_back_to_module_on_file_not_found(self, mock_popen: MagicMock) -> None:
        proc = MagicMock()
        proc.pid = 42

        def popen_side_effect(cmd, **kwargs):
            if cmd[0].startswith("mars-agent-"):
        ...
        spec = _make_spec()
        pid, _workdir, via_module = _launch_service_agent(spec, "localhost:7432")

        assert pid == 42
        assert via_module is True
...
class TestAutoSpawnFreeAgents:
    @patch("mars.cli.service_manager._launch_service_agent")
    @patch("mars.cli.service_manager.all_specs")
    def test_spawns_only_free_agents(
        ...
        mock_launch.return_value = (1234, Path("artifacts/clock"), False)
    ...
    def test_returns_list_of_pids(
        ...
        mock_launch.side_effect = [
            (101, Path("artifacts/clock"), False),
            (102, Path("artifacts/profiler"), False),
        ]
        pids = _auto_spawn_free_agents("localhost:7432")
