...
from mars.runtime.agents import status_agent
...
class TestRestUnreachable:
    def test_http_get_json_translates_connection_refused(self, monkeypatch) -> None:
        ...
        monkeypatch.setattr(status_agent.urllib.request, "urlopen", _raise)
        result = status_agent._http_get_json("http://localhost:7433/agents")
    ...
    def test_collect_status_collapses_to_top_level_diagnostic(self, monkeypatch) -> None:
        ...
        monkeypatch.setattr(status_agent.urllib.request, "urlopen", _raise)
        snap = status_agent.collect_status("http://localhost:7433")
