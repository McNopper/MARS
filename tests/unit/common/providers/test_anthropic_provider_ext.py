"""Extended unit tests for mars.common.providers.anthropic.AnthropicService.
...
def _make_provider(
    ...
    **kwargs,
    ...
    """Construct an AnthropicService with a patched SDK client."""
    with mock.patch.dict(os.environ, {"ANTHROPIC_API_KEY": api_key}):
        with mock.patch("anthropic.AsyncAnthropic"):
            ...
            return AnthropicService(api_key=api_key, **kwargs)
...
class TestAnthropicServiceConstruction:
    ...
    def test_init_reads_api_key_from_env(self) -> None:
        with mock.patch.dict(os.environ, {"ANTHROPIC_API_KEY": "sk-from-env"}):
            with mock.patch("anthropic.AsyncAnthropic") as mock_client:
                ...
                assert mock_client.call_args.kwargs["api_key"] == "sk-from-env"

    def test_init_accepts_thinking_flag(self) -> None:
        p = _make_provider(thinking=True)
        assert p._thinking is True
...
class TestToolSchema:
    def test_uses_input_schema_key(self) -> None:
        ...
        schema = AnthropicService._tool_schema(spec)
...
class TestParseResponse:
    def test_maps_text_block_to_content(self) -> None:
        ...
        result = AnthropicService._parse_response(resp)
...
class TestMergeExtraHeaders:
    def test_merges_new_key(self) -> None:
        from mars.server.services.llm.anthropic import AnthropicService
        result = AnthropicService._merge_extra_headers(None, {"X-Custom": "val"})
        assert result["X-Custom"] == "val"

    def test_anthropic_beta_deduplicates_values(self) -> None:
        from mars.server.services.llm.anthropic import AnthropicService
        existing = {"anthropic-beta": "value-a"}
        result = AnthropicService._merge_extra_headers(existing, {"anthropic-beta": "value-b"})
    ...
    def test_anthropic_beta_does_not_duplicate_same_value(self) -> None:
        from mars.server.services.llm.anthropic import AnthropicService
        existing = {"anthropic-beta": "value-a"}
        result = AnthropicService._merge_extra_headers(existing, {"anthropic-beta": "value-a"})
        assert result["anthropic-beta"].count("value-a") == 1

    def test_none_current_treated_as_empty(self) -> None:
        from mars.server.services.llm.anthropic import AnthropicService
        result = AnthropicService._merge_extra_headers(None, {"anthropic-beta": "b1"})
