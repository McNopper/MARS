...
from mars.client.cli.main import (
    _cmd_context,
    _cmd_new,
    _cmd_rewind,
    _cmd_search,
...
def _render_sidebar(state: MARSState, width: int = 40) -> str:
    """Render sidebar to plain ASCII string."""
...
class TestHandleCommandLocal:
    ...
    def test_new_clears_chat_history(self):
        state = _make_state_with_agent()
        state.agents["llm.mock@1"].chat.append(
            __import__("mars.client.cli.models", fromlist=["ChatMessage"]).ChatMessage(
...
# ---------------------------------------------------------------------------
# Thinking spinner in sidebar ASCII render
# ---------------------------------------------------------------------------


class TestThinkingSpinner:
    def test_thinking_dot_appears_in_sidebar(self):
        state = _make_state_with_agent()
        state.agents["llm.mock@1"].fsm_state = "THINKING"

        output = _render_sidebar(state, width=60)

        # Spinner cycles through frames — at least one frame character must appear
        spinner_chars = MARSRenderer._THINKING_SPINNER
        assert any(ch in output for ch in spinner_chars), (
            f"No thinking spinner character found in sidebar output.\n{output}"
        )

    def test_idle_agent_has_no_spinner(self):
        state = _make_state_with_agent()
        state.agents["llm.mock@1"].fsm_state = "IDLE"

        output = _render_sidebar(state, width=60)

        # Spinner must NOT appear for a non-THINKING agent
        spinner_chars = MARSRenderer._THINKING_SPINNER
        # The spinner chars are generic so we check the bullet ● (non-spinner)
        assert "●" in output

    def test_thinking_agent_name_in_sidebar(self):
        state = _make_state_with_agent("llm.thinking@1")
        state.agents["llm.thinking@1"].fsm_state = "THINKING"

        output = _render_sidebar(state, width=60)
        assert "llm.thinking@1" in output

    def test_thinking_spinner_changes_frame(self):
        """Successive renders at different times produce different spinner frames."""
        frames = set()
        for t_offset in range(len(MARSRenderer._THINKING_SPINNER)):
            state = _make_state_with_agent()
            state.agents["llm.mock@1"].fsm_state = "THINKING"
            renderer = MARSRenderer(state)
            # Patch time.monotonic to a specific value so we control the frame
            with patch("mars.client.cli.renderer.time") as mock_time:
                mock_time.monotonic.return_value = t_offset / 8.0
                buf = StringIO()
                from rich.console import Console
                console = Console(file=buf, force_terminal=True, width=60, color_system=None)
                console.print(renderer.render_sidebar())
            output = buf.getvalue()
            for ch in MARSRenderer._THINKING_SPINNER:
                if ch in output:
                    frames.add(ch)
        assert len(frames) > 1, "Thinking spinner must rotate through multiple frames"
