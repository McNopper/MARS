"""Unit tests for mars.client.cli.utils."""
...
from mars.client.cli.models import AgentRecord, MARSState
...
class TestNormalizeAgentType:
    ...
    def test_service_proxy_becomes_service_agent(self) -> None:
        assert _normalize_agent_type("ServiceProxyAgent") == "ServiceAgent"
...
class TestRunningServiceAgentNames:
    ...
    def test_llm_agents_excluded(self) -> None:
        state = self._state_with({"llm.1": "LLMAgent", "clock-agent": "ServiceAgent"})
        names = _running_service_agent_names(state)
        assert "llm.1" not in names

    def test_service_agents_included(self) -> None:
        state = self._state_with({"clock-agent": "ServiceAgent"})
        names = _running_service_agent_names(state)
        assert "clock-agent" in names
    ...
    def test_no_suffix_strip_for_non_agent_suffix(self) -> None:
        state = self._state_with({"profiler": "ServiceAgent"})
        names = _running_service_agent_names(state)
