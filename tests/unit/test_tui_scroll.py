...
"""
import pytest
from mars.cli.main import _nav_sidebar, _sync_sidebar_cursor
...
def _apply_scroll_key(state: _FakeState, sc: str) -> None:
    """Mirrors the _scroll_key Windows inner function in mars/cli/main.py."""
    UP = ('H',)
    DOWN = ('P',)
...
def _apply_unix_scroll(state: _FakeState, ch3: str) -> None:
    """Mirrors the Unix ESC[ escape handler in mars/cli/main.py."""
    focus = state.panel_focus
    if ch3 == 'A':
        if focus == "chat":
            state.chat_scroll += 1
