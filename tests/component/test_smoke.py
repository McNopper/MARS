...
in ``mars.server.main`` — a bug that only surfaces at runtime when ``_async_server``
...
def test_srv_main_imports():
    """mars.server.main must import without errors."""
    module = importlib.import_module("mars.server.main")
...
def test_scope_store_imports():
    """mars.server.storage.scopes.store must import without errors."""
    module = importlib.import_module("mars.server.storage.scopes.store")
    assert module is not None
...
def test_scope_store_instantiation(tmp_path: Path):
    ...
    failed in mars.server.main when ``Path`` was not imported."""
...
def test_mars_client_terminal_apply_event_despawn():
    ...
    state.agents["temp-agent"] = AgentRecord(
        agent_id="temp-agent", agent_type="Provider",
