...
```bash
python examples/hello_mars.py
```

Spawns a single mock agent, sends a message, and prints the response — no API key needed.
...
Renders the platform in-process using the mock provider, captures three frames at real timestamps, and writes them to `screenshots/` (gitignored) as PNG, SVG, and TXT.
