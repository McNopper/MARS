...
mars/
  ...
  providers/    LLM provider adapters + registry (mock, anthropic, github-models, copilot, ollama)
...
scripts/        Utility scripts (capture_screenshot.py renders CLI frames to screenshots/)
...
- Group related tests in classes (`class TestStateMachine:`, `class TestPolicyEngine:`, …).
...
- Always clean up platform resources; use `async with Platform() as p:` or `await p.shutdown()` in teardown.

### Minimal async test example

```python
import pytest
from mars import Platform
from mars.providers.mock import MockProvider

class TestMyFeature:
    async def test_something(self):
        provider = MockProvider()
        async with Platform() as platform:
            # ... spawn agent, send message, assert result
            assert True
```
