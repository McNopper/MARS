...
from mars.cli.models import AgentRecord, ChatMessage, FeedItem, MARSState, DEFAULT_PORT, DEFAULT_FEDERATION_PORT
from mars.cli.service_manager import _auto_spawn_free_agents, _launch_service_agent
from mars.cli.utils import _load_dotenv, _local_ip, _normalize_agent_type
from mars.scopes.store import ScopeStore
from mars.services.registry import get as get_agent_spec
...
class MARSServer:
    ...
    async def start_mcp_agents(self) -> None:
        """Start all MCP-protocol service agents as stdio subprocesses."""
        import shlex
        import sys
        ...
        for spec in all_specs():
            ...
            try:
                ...
                # Fall back to python -m if the entrypoint isn't installed
                try:
                    import shutil
                    ...
                    ep = cmd[0] if cmd else f"mars-agent-{spec.name}"
                    module = "mars.services." + ep.replace("mars-agent-", "").replace("-", "_") + "_agent"
                    command = [sys.executable, "-m", module] + cmd[1:]
                ...
                logger.info("MCP agent started: %s (%s)", agent_id, spec.name)
            except Exception as exc:
                logger.warning("Could not start MCP agent %s: %s", spec.name, exc)
    ...
    async def _spawn_from_tokens(self, tokens: list[str]) -> str:
        ...
        if spec is not None:
            pid, _workdir, _via_module = _launch_service_agent(spec, self._server_addr, tokens[1:])
...
class MARSRestAPI:
    ...
    async def serve(self, host: str, port: int = 7433) -> None:
...
class MARSWebSocketServer:
    ...
    async def serve(self, host: str, port: int = 7434) -> None:
...
async def _async_server(args: argparse.Namespace) -> None:
    ...
    try:
        from mars.providers.copilot import _resolve_token as _copilot_token
    except ImportError:
...
def main(argv=None):
    ...
    parser.add_argument("--http-port", type=int, default=7433, dest="http_port")
    parser.add_argument("--ws-port", type=int, default=7434, dest="ws_port")
