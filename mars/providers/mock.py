...
from mars.providers.base import LLMMessage, LLMProvider, LLMResponse, ToolSpec
    ...
    async def complete(
        ...
        **_: Any,
        ...
        if self._turn == 1 and tools:
            ...
            if trigger_matches:
                if self._tool_name:
                    tool = next((t for t in tools if t.name == self._tool_name), tools[0])
                else:
                    tool = tools[0]
                ...
                return LLMResponse(
                    ...
                    tool_calls=[{
                        ...
                        "function": {
                            "name": tool.name,
                            "arguments": _json.dumps({"request": self._tool_request}),
                        },
                    }],
