...
The one exception is the **launcher agent** (`svc.launcher`), which must hold a live TCP connection to send server commands, and is therefore a TCP wire agent.
...
Each spawned agent is a full LLM wire agent that participates in rooms, skill routing, tool calls, artifacts, and federation message routing.
...
|------|------|-------------|
| 🤖 | `LLMAgent` | LLM wire agent — Anthropic, Copilot, Ollama, GitHub Models |
