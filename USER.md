...
/spawn github-models             # free LLM (needs GITHUB_TOKEN)
...
/spawn screenshot                # built-in service agent
...
/spawn github-models --name reviewer.1 --role "Reviewer" --goal "Triage service-agent replies" --behaviour reactive
```
...
/models github-models            # list GitHub Models catalogue
...
The `bridge.copilot@1` agent surfaces **GitHub Copilot Chat** (gpt-4o) as a first-class MARS participant. It is auto-spawned on server start and appears with the 🌉 bridge icon in the sidebar.

**Prerequisites:** the `gh` CLI must be installed and authenticated:
...
```text
/switch bridge.copilot@1      # focus the Copilot bridge
hello, what is MARS?          # any message → Copilot AI response in chat
clear                         # reset conversation history
```

Conversation history (last 20 messages per sender) is kept so follow-up questions have context. If the `gh` CLI is not authenticated, the bridge replies with an error message explaining what to do.
...
Service agents (screenshot, profiler, ocr, …) return their results as artifacts that appear on the capability board.
...
```text
/federation                      # show connected MARS platforms
```
...
| `list_skills` tool | LLM discovers available service skills |
| `use_skill` tool | LLM invokes a service agent by skill name |
| `/federation` | Multi-platform status |
